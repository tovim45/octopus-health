...
csv_file = "C:\\automation\\octopus\\src\\test_data\\medications.csv"
output_result_file = "C:\\automation\\octopus\\reports\\result.json"
...
def test_validate_value_of_medicine_from_clalit_portal():
    ...
    for medicine in medicines.index:
        ...
        result = mp.get_medicine_search_results_list()
        if result == '0':
            result_data = {medicine:  int(result)}
            print(result_data)
            write_output_file(result_data)
        mp.click_x_search()
